    ...
    <x-ui.table-card title="Cuentas disponibles para cobro">
        <x-slot:actions>
            <form class="d-flex gap-2" method="GET" action="{{ route('rectorate.collectible-accounts.index') }}" role="search">
                <label class="visually-hidden" for="account-search">Buscar estudiante, CI o matrícula</label>
                <input class="form-control" id="account-search" name="buscar" type="search" value="{{ $search }}" placeholder="Estudiante, CI o matrícula" autocomplete="off">
                <button class="btn btn-primary" type="submit"><i class="ti ti-search me-1" aria-hidden="true"></i>Buscar</button>
            </form>
        </x-slot:actions>

        <table class="table table-hover align-middle mb-0">
            <thead><tr><th>Matrícula</th><th>Estudiante</th><th>Programa / Plan</th><th>Estado</th><th class="text-end">Saldo</th><th class="text-end">Acción</th></tr></thead>
            <tbody>
                @forelse ($contracts as $contract)
                    @php($balance = (float) $contract->total_charged - (float) $contract->total_paid)
                    <tr>
                        <td><span class="fw-semibold">{{ $contract->account_number }}</span><small class="d-block text-body-secondary">{{ $contract->application->campus?->name }}</small></td>
