    ...
    public function reverse(SinInvoiceIssue $invoice, SinCufd $currentCufd): InvoiceSiatResponse
    {
        ...
        try {
            ...
            throw new InvoiceTransportException(
                $this->sanitizer->text($exception->getMessage(), (string) $token?->api_token) ?? 'Error al comunicar la reversión al SIN.',
                isset($client), $this->classifier->classify($exception), $exception,
            );
        }
